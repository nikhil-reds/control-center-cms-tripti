...
function MediaCard({ mode, title, description, count, active, pending, onCommand, slidePosition }: { mode: SelectableMode; title: string; description: string; count: number; active: boolean; pending: boolean; onCommand: (body: Record<string, string>) => Promise<void>; slidePosition?: string }) {
  ...
  return <article className={`${styles.mediaCard} ${active ? styles.mediaCardActive : ""}`}>
    ...
    <div className={styles.powerRow}><button className={`${styles.onButton} ${active ? styles.onButtonActive : ""}`} aria-pressed={active} disabled={!available || pending || active} onClick={() => void onCommand({ action: "activate", mode })}><PowerIcon/>On</button><button className={styles.offButton} aria-pressed={!active} disabled={pending || !active} onClick={() => void onCommand({ action: "deactivate" })}><PowerIcon off/>Off</button></div>
